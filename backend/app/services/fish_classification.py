...
# Alternate path in case the standard path doesn't work
ALT_MODEL_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                            'app', 'models', 'fish_classification', 'fish_classifier_model.h5')
...
# Alternate path for class names
ALT_CLASS_NAMES_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                   'app', 'models', 'fish_classification', 'class_names.txt')
...
try:
    ...
    # If that fails, try the alternate path
        ...
        print(f"Using alternate model path: {ALT_MODEL_PATH}")
        model = tf.keras.models.load_model(ALT_MODEL_PATH)
    else:
    ...
    if os.path.exists(CLASS_NAMES_PATH):
        ...
        # Modify the function to use the alternate path
        with open(ALT_CLASS_NAMES_PATH, 'r') as f:
            class_names = {}
            for line in f:
                idx, name = line.strip().split(',', 1)
                class_names[int(idx)] = name
